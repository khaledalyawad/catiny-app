...
const { getAlbum, getAllAlbums, updateAlbum, deleteAlbum, searchAlbums } = AlbumSagas;
...
test('search success path', () => {
  const response = FixtureAPI.searchAlbums();
  const step = stepper(searchAlbums(FixtureAPI, '*'));
  // Step 1: Hit the api
  step();
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(AlbumActions.albumSearchSuccess([{ id: 1 }, { id: 2 }])));
});
...
test('search failure path', () => {
  ...
  step();
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(AlbumActions.albumSearchFailure()));
});
