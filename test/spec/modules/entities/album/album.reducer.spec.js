...
test('attempt searching a album', () => {
  const state = reducer(INITIAL_STATE, Actions.albumSearchRequest(1));
...
test('success searching a album', () => {
  const state = reducer(INITIAL_STATE, Actions.albumSearchSuccess({ id: 1 }));

  expect(state.searching).toBe(false);
  expect(state.errorSearching).toBe(null);
  expect(state.albumList).toEqual({ id: 1 });
});
...
test('failure searching a album', () => {
  const state = reducer(INITIAL_STATE, Actions.albumSearchFailure({ error: 'Not found' }));
