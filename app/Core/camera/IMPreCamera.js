...
function IMPreCamera(props) {
  ...
  const flashModeIcon = {
    0: require('../../CoreAssets/flash-on.png'),
    1: require('../../CoreAssets/flash-auto.png'),
  ...
  return (
    <View style={styles.container}>
      ...
      {useExternalSound && !isRecording && (
        <TouchableOpacity onPress={onSoundPress} style={styles.headerTitleContainer}>
          <Image source={require('../../CoreAssets/musical-notes.png')} style={styles.optionIcon} />
      ...
      {useExternalSound && !isRecording && (
        <View style={styles.optionsContainer}>
          <TouchableOpacity onPress={onCameraFlip} style={styles.optionItemContainer}>
            <Image source={require('../../CoreAssets/camera-rotate.png')} style={styles.optionIcon} />
            <Text style={styles.iconTitle}>{'Flip'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setIsSpeedVisible(!isSpeedVisible)} style={styles.optionItemContainer}>
            <Image source={require('../../CoreAssets/speed.png')} style={styles.optionIcon} />
            <Text style={styles.iconTitle}>{'Speed'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onFlashToggle} style={styles.optionItemContainer}>
            <Image source={flashModeIcon[flashMode]} style={styles.optionIcon} />
          ...
          {videoSpeedConfig.map((speedConfig, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                key={`${index}`}
                style={[styles.speedItemContainer, isSelectedSpeed && styles.selectedSpeedItemContainer]}>
                <Text style={[styles.speedTitle, isSelectedSpeed && styles.selectedSpeedTitle]}>{speedConfig.title}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
      ...
      <View style={styles.control}>
        ...
        <TouchableOpacity onPress={onOpenPhotos}>
          <Image source={require('../../CoreAssets/library-landscape.png')} style={styles.imageIcon} />
        </TouchableOpacity>
      </View>
    </View>
  );
