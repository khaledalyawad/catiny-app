...
export default class extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (stories !== prevProps.stories || userStories !== prevProps.userStories) {
  ...
  render() {
    const shouldOpenCamera = store.userStory.items && store.userStory.items.length > 0 ? false : true;
    return (
      <View style={styles.container}>
        ...
        />
        <FullStoriesModal isModalOpen={store.carouselOpen} onClosed={this.onClose} />
      </View>
    );
  }
}
