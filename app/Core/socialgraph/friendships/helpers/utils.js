...
export const getFollowerIDs = (friendships, friends, followEnabled) =>
{
  ...
  return friendships
    .filter(
      (friendship) =>
        friendship.type == FriendshipConstants.FriendshipType.inbound || friendship.type == FriendshipConstants.FriendshipType.reciprocal,
    )
...
export const filteredNonFriendshipsFromUsers = (keyword, users, friendships) =>
{
  ...
  if (keyword && keyword.length > 0)
  {
    filteredUsers = users.filter((user) =>
    {
      ...
      return fullName && fullName.toLowerCase().indexOf(keyword.toLowerCase()) >= 0;
    });
  }
