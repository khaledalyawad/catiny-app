...
import {filteredNonFriendshipsFromUsers, FriendshipConstants, IMFriendItem} from '../..';
...
function IMUserSearchModal(props)
{
  ...
  useEffect(() =>
  {
    ...
    friendshipsTracker.current = new FriendshipAPITracker(store, currentUser.id, followEnabled, followEnabled, followEnabled);
  ...
  const onAddFriend = (item, index) =>
  {
    if (item.type == FriendshipConstants.FriendshipType.none)
    {
      ...
      friendshipsTracker.current?.addFriendRequest(currentUser, item.user, (response) =>
      {
        if (response?.error)
        {
  ...
  return (
    <View style={styles.container}>
      ...
      {(reduxUsers == null || friendships == null) && <ActivityIndicator style={{marginTop: 15}} size='small' color='#cccccc' />}
      <FlatList keyboardShouldPersistTaps='always' data={filteredFriendships} renderItem={renderItem} />
    </View>
  );
}
