...
export default class FriendshipTracker
{
  ...
  subscribeIfNeeded = () =>
  {
    ...
    if (!state.friends.didSubscribeToFriendships)
    {
      ...
      this.inboundFriendshipsUnsubscribe = friendshipAPI.subscribeToInboundFriendships(userId, this.onInboundFriendshipsUpdate);
      this.outboundFriendshipsUnsubscribe = friendshipAPI.subscribeToOutboundFriendships(userId, this.onOutboundFriendshipsUpdate);
    }
  };
  ...
  hydrateFriendships()
  {
    ...
    if (inboundFriendships && outboundFriendships && bannedUserIDs)
    {
      ...
      var finalFriendships = [...friendshipsFromInbound, ...friendshipsFromFriends, ...friendshipsFromOutbound].filter(
        (friendship) => !bannedUserIDs.includes(friendship.user.id),
      );
