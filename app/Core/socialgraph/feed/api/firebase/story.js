...
export const hydrateStoriesForNewFriendship = async (destUserID, sourceUserID) =>
{
  ...
  const storiesDestRef = firebase.firestore().collection('social_feeds').doc(destUserID).collection('stories_feed');

  const unsubscribeToSourceStories = storiesRef.where('authorID', '==', sourceUserID).onSnapshot(
    (querySnapshot) =>
    {
      querySnapshot.forEach((doc) =>
      {
...
export const removeStoriesForOldFriendship = async (destUserID, oldFriendID) =>
{
  ...
  const unsubscribeToSourceStories = storiesRef.where('authorID', '==', oldFriendID).onSnapshot(
    (querySnapshot) =>
    {
      querySnapshot.forEach((doc) =>
      {
