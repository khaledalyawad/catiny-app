...
export const reactionsRef = firebase.firestore().collection('socialnetwork_reactions');
...
export const handleReaction = async (reaction, user, post, followEnabled) => {
  try {
    ...
    if (documentSnapshots.docs.length > 0) {
      ...
      notificationManager.sendPushNotification(post.author, IMLocalized('Instamobile'), message, 'social_reaction', {
        outBound: user,
        reaction,
      });
    }
