...
function IMRichTextInput(props)
{
  ...
  const onChange = (input, fromAtBtn) =>
  {
    ...
    if (text.length < prevText.length)
    {
      ...
      const totalSelection = {
        ...
        end: charDeleted > 1 ? selectionCopy.start + charDeleted : selectionCopy.start,
      };
      /**
  ...
  const onContentSizeChange = (evt) =>
  {
    ...
    if (evt)
    {
      ...
      const height = Platform.OS === 'ios' ? evt.nativeEvent.contentSize.height : evt.nativeEvent.contentSize.height - androidTextHeight;
      let editHeight = 40;
