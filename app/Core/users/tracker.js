...
export default class UsersTracker {
  ...
  subscribeIfNeeded = () => {
    ...
    if (!state.users.didSubscribeToUsers) {
      ...
      this.usersUnsubscribe = userAPIManager.subscribeUsers(this.onUsersCollection);
      this.abusesUnsubscribe = reportingManager.unsubscribeAbuseDB(this.viewerID, this.onAbusesUpdate);
    }
  };
  ...
  updateUsersIfNeeded = () => {
    ...
    if (bannedUserIDs && users) {
      const filteredUsers = users.filter((user) => !bannedUserIDs.includes(user.id));
      this.unsubscribe();
