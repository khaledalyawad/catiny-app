import {setChannels, setChannelsSubcribed} from '../../redux';
...
export default class FirebaseChannelsTracker
{
  ...
  subscribeIfNeeded = () =>
  {
    ...
    if (!state.chat.areChannelsSubcribed)
    {
      ...
      this.channelsUnsubscribe = subscribeChannels(userId, this.onChannelCollectionUpdate);
    }
  };
  ...
  channelsWithNoBannedUsers = (channels, bannedUserIDs) =>
  {
    ...
    channels.forEach((channel) =>
    {
      if (
        ...
        (!bannedUserIDs || channel.participants.length != 1 || !bannedUserIDs.includes(channel.participants[0].id))
      )
      {
  ...
  updateChannelsIfNeeded = () =>
  {
    ...
    const channels = this.channelsWithNoBannedUsers(this.channels, this.bannedUserIDs);
