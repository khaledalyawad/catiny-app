...
export const hydrateSocialChatFeedItem = async (sender, channel, message, createdAt) =>
{
  ...
  const feedItemTitleForSender =
    otherParticipants?.length == 1 ? otherParticipants[0].firstName + ' ' + otherParticipants[0].lastName : channel.name;
  const feedItemTitleForRecipients = otherParticipants?.length == 1 ? sender.firstName + ' ' + sender.lastName : channel.name;
  ...
  otherParticipants.forEach((recipient) =>
  {
    ...
    socialFeedsRef.doc(recipient.id).collection('chat_feed').doc(channel.id).set(
      {
        id: channel.id,
