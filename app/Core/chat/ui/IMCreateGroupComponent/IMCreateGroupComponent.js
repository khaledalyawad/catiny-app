...
function IMCreateGroupComponent(props) {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => onCheck(item)} style={styles.itemContainer}>
      ...
      <View style={styles.addFlexContainer}>{item.checked && <Image style={styles.checked} source={appStyles.iconSet.checked} />}</View>
    ...
    <View style={styles.container}>
      ...
      {friends && friends.length <= 1 && (
        <View style={styles.emptyViewContainer}>
          <TNEmptyStateView appStyles={appStyles} emptyStateConfig={emptyStateConfig} />
        </View>
      )}
      <DialogInput
